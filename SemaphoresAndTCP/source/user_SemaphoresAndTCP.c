...
void ourclk(void) {
    ...
    if ((clkCount % 250) == 0) {
        //LCDPrintfLine(1, "Robot ready!");
        //LCDPrintfLine(2, "Clock: %d", clkCount);
    }
}
...
void ComWithLinux(void) {
    ...
    while(1) {
        ...
        if (GET_LVDATA_FROM_LINUX == 1) {
            Cache_inv((void *)ptrshrdmem,sizeof(sharedmemstruct), Cache_Type_ALL, EDMA3_CACHE_WAIT);

            int i;
            for (i=0; i<ptrshrdmem->DSPRec_size; i++) {
                mystring[i] = ptrshrdmem->DSPRec_buf[i];
            }
            CLR_LVDATA_FROM_LINUX;

            Semaphore_post(semaphore0);
            LCDPrintfLine(1, "%s", mystring);
            LCDPrintfLine(2, "task count: %d", task0Count);
            int len = sprintf(TXstring, "%s count = %d, task count = %d", mystring, clkCount, task0Count);

            if (GET_LVDATA_TO_LINUX == 1) {
                for (i=0; i<len; i++) {
                    ptrshrdmem->DSPSend_buf[i] = TXstring[i];
                }
                ptrshrdmem->DSPSend_size = len;
