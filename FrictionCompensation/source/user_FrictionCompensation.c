...
extern int adc1_i2c,adc2_i2c,adc3_i2c,adc4_i2c,adc5_i2c,adc6_i2c,adc7_i2c,adc8_i2c;
extern int CompassNew;
extern int ir1_i2c,ir2_i2c,ir3_i2c,ir4_i2c,ir5_i2c;

extern int SPIenc_state;
extern long SPIenc1_reading,SPIenc2_reading,SPIenc3_reading,SPIenc4_reading;
...
extern float mcbsp_pwm1,mcbsp_pwm2;
extern float mcbsp_dac1,mcbsp_dac2;
...
// Calculated coulomb friction
float Cpos = 2.9;
float Cneg = -2.76;
// Calculated viscous friction
float Vpos = 1.96;
float Vneg = 2.04;
...
// Saturates rads to [-10, 10]
float minMax(float rads) {
    if (rads > 10) {
        return 10;
    }
    if (rads < -10) {
...
void control(void) {
    ...
    // Read from motor's optical encoders
    float p_curr_1 = Enc1_rad;
    float p_curr_2 = Enc2_rad;

    // Add your code here in between updateData and sendData
    // Send encoder data to PWM outputs 4 and 5
    PWM_out(EPWM4, Enc1_rad);
    PWM_out(EPWM5, Enc2_rad);
    // Send scaled [0, 2.5V] encoder data to DAC
    writeDAC7564((Enc1_rad + 10)/8, (Enc2_rad + 10)/8);

    // discrete approx of derivative to calc linear vel in ft/s
    v1 = ((p_curr_1-p_old_1)/0.001)/193;
    v2 = ((p_curr_2-p_old_2)/0.001)/193;

    // initalize control to 0 so that only friction compensation is acting
    u1 = 0;
    u2 = 0;
    ...
    timeint++;
    if (timeint % 100 == 0) {
        //LCDPrintfLine(1, "%ld ms passed", timeint);
        //LCDPrintfLine(2, "enc1=%0.1f enc2=%0.1f", Enc1_rad, Enc2_rad);
        //LCDPrintfLine(1, "v1 = %0.1f v2 = %0.1f", v1,v2);
        LCDPrintfLine(1, "avg v = %0.01f", (v1+v2)/2);
        LCDPrintfLine(2, "Input = %0.1f", u1);
