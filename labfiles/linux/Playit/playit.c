...
//	run_server();
	...
	while (!gs_quit) {
		sched_yield();
		if  (GPIO_getOutput(mygpio,DATAFORFILE_TO_LINUX_BANK,DATAFORFILE_TO_LINUX_FLAG) == 0) { // New Response data from DSP    


			if (myshared->scratch[0] == 2.0) {
				printf("/home/root/wavfiles/robotspeak.sh \"This is a test\"");
				size_buff = sprintf(buffer4SystemCall,"/home/root/wavfiles/robotspeak.sh \"This is a test\"");
				if (size_buff < 512) {
