   ...
   switch ((uint32_t)uart)
      {
         ...
         case UART1_REG_BASE:
        	// set baud rate...assumes default 16x oversampling.
        	// Changed by Dan Block,  UBL 03.20.00.12 switches ASYNC3 clk source to PLL1 CLK S2 which is at 132MHz for DDR
        	//  ASYNC3 is used for UART1 and UART2
        	divisor = 132000000 / (baud_rate * 16);
            break;
