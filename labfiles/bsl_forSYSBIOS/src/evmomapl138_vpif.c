...
//-----------------------------------------------------------------------------
uint32_t VPIF_initReceive_withPRU(video_connector_e in_conn)
{
...
//         SETBIT(VPIF->CH1CTRL, CHANEN);
      ...
      case VIDEO_CONN_CAMERA:
		 ...
		 // Dan Block changed to 8BPP for 0v6620 camera
         VPIF->CH0CTRL = DATAWIDTH_8BPP | (2 << INTLINE_SHIFT) | 0x4000 | FIELDFRAME | CH01_INTRPROG | INTFRAME_TOP_ONLY | CAPMODE;
//		 VPIF->CH0CTRL = DATAWIDTH_10BPP | (120 << INTLINE_SHIFT) | FIELDFRAME | CH01_INTRPROG | INTFRAME_TOP_ONLY | CAPMODE;
		 ...
		 // Dan Block changed to 352 for OV6620 camera
		 VPIF_CAP_CH0->IMGOFFSET = 352;

         // config interrupts.
//         SETBIT(VPIF->INTEN, INT_FRAME_CH0);
//	     SETBIT(VPIF->INTSTATCLR, INT_FRAME_CH0);
//		 SETBIT(VPIF->INTENSET, INT_FRAME_CH0);
         SETBIT(VPIF->INTEN, INT_FRAME_CH1);
	     SETBIT(VPIF->INTSTATCLR, INT_FRAME_CH1);
		 SETBIT(VPIF->INTENSET, INT_FRAME_CH1);


//		 SETBIT(VPIF->INTEN, INT_FRAME_CH0 | INT_FRAME_CH1);
//         SETBIT(VPIF->INTENCLR, INT_FRAME_CH0 | INT_FRAME_CH1);
//         SETBIT(VPIF->INTSTATCLR, INT_FRAME_CH0 | INT_FRAME_CH1);
...
uint32_t VPIF_initReceive(video_connector_e in_conn)
{
   ...
   switch (in_conn)
   {
      ...
      case VIDEO_CONN_CAMERA:
         ...
         // config channel 0 & 1 control and capture registers.
		 // Dan Block changed to 8BPP for 0v6620 camera
         VPIF->CH0CTRL = DATAWIDTH_8BPP | (288 << INTLINE_SHIFT) | 0x4000 | FIELDFRAME | CH01_INTRPROG | INTFRAME_TOP_ONLY | CAPMODE;
//		 VPIF->CH0CTRL = DATAWIDTH_10BPP | (120 << INTLINE_SHIFT) | FIELDFRAME | CH01_INTRPROG | INTFRAME_TOP_ONLY | CAPMODE;
         VPIF->CH1CTRL = FIELDFRAME | CH01_INTRPROG | INTFRAME_TOP_ONLY | CAPMODE;
