...
;-------------------------------------------------------------------------+
                                        ;                                 |
        .text                           ; program                         |
        .global _atandp, _atan          ; entry labels for arcTan (Z)     |
        .global _atan2dp;,_atan2         ; entry labels for arcTan2(Y,X)   |
        .global _recip,  _recipdp       ; DP FP reciprocal entry labels   |
        ;.global _divdp,  __divd         ; DP FP divide entry labels       |
        .global _divdp,  __divd         ; DP FP divide entry labels       |
        ...
        .align  32                      ; fetch packet boundary           |
                                        ;                                 |
;--------------DP FP atan(Z)----------------------------------------------+
;_atan:                                  ; rts entry for DP FP atan(Z)
...
* ======================================================================== *
_atan2:                                 ; rts entry for DP FP atan2(Y,X)
...
|| [B0] stw		B6, *A0			; _errno = 0 if (Y,X) not eq (0,0)
  ...
  [!A2] b       .s2     _atandp                 ; Z is NZ, make near arcTan(Z) call
        mvkl    .s2     BK8,b3
        mvkh    .s2     BK8,b3
