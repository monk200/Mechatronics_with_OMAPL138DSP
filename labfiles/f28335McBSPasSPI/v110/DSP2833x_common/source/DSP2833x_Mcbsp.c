...
//#endif
	...
	#define CPU_SPD              150E6
	#define MCBSP_SRG_FREQ       CPU_SPD/4  // SRG input is LSPCLK SYSCLKOUT use SYSCLKOUT/4 for longer delay
#else
...
// original settings
//#define CLKGDV_VAL         1
//#define CLKGDV_VAL           14
#define CLKGDV_VAL           20
...
//	McbspaRegs.SPCR1.bit.DLB = 1;       // Enable loopback mode for test. Comment out for normal McBSP transfer mode.
	...
	McbspaRegs.SRGR2.bit.FPER = 0;		// FPER = 1 CLKG periods
...
//	McbspbRegs.SRGR2.bit.FPER = 0;		// FPER = 1 CLKG periods
//
...
// McBSP-B register settings
    ...
    McbspbRegs.SPCR1.bit.CLKSTP = 0x3;// or could be 0x3
    McbspbRegs.PCR.bit.CLKXP = 0;  //or could be 1
    McbspbRegs.PCR.bit.CLKRP = 1;  // or could be 1
    McbspbRegs.PCR.bit.CLKXM = 0;       // CLKX generated internally SPI Master
    McbspbRegs.PCR.bit.SCLKME = 0;
    McbspbRegs.SRGR2.bit.CLKSM = 1;     // CLKSM=1 (If SCLKME=0, i/p clock to SRG is LSPCLK)
    McbspbRegs.SRGR1.bit.CLKGDV = 1; //CLKGDV_VAL;   // CLKG frequency = LSPCLK/(CLKGDV+1)
    McbspbRegs.PCR.bit.FSXM = 0;        // FSX generated internally, FSR derived from an external source
    McbspbRegs.SRGR2.bit.FSGM = 0;
    McbspbRegs.PCR.bit.FSXP = 1;
    McbspbRegs.XCR2.bit.XDATDLY = 0x0;
    McbspbRegs.RCR2.bit.RDATDLY = 0x0;
    McbspbRegs.SRGR2.bit.FPER = 0;      // FPER = 1 CLKG periods
    McbspbRegs.SRGR1.bit.FWID = 0;              // Frame Width = 1 CLKG period
    McbspbRegs.MCR2.all = 0x0;
    McbspbRegs.MCR1.all = 0x0;
