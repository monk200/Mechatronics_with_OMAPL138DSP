...
// Example: CheckDog:
//---------------------------------------------------------------------------
// This function returns the WDFLAG bit.

unsigned char CheckDog(void)
{
	//bit 7 of WDCR is WDFLAG

	//WDFLAG == 0 implies a reset from either XRS, or power-up
	//WDFLAG == 1 implies a reset from the WDRST signal
	//WDFLAG remains 0 until a 1 is written to it. Writes of 0 are ignored.

	unsigned char state = 0;
	state = (SysCtrlRegs.WDCR&0x0080)>>7;
	if (state) {
		EALLOW;
