...
volatile int linux_noimagefound = 1;
volatile float 	object_x 			= 0.0;
volatile float  linux_object_x      = 0.0;
volatile float 	object_y 			= 0.0;
volatile float  linux_object_y      = 0.0;
volatile int 	new_coordata 		= 0;
volatile int numpels = 0;
volatile int linux_numpels = 0;
volatile int new_num_found_objects = 0;
volatile int linux_new_num_found_objects = 0;
volatile float   new_object_theta 	= 0.0;
volatile float   new_C20				= 0.0;
volatile float   new_C02				= 0.0;

volatile int new_sendtolinux = 0;
volatile float send_object_x = 0;
volatile float send_object_y = 0;
volatile int send_numpels = 0;
...
//            }
		...
		// Flag ComWithLinux() TSK new data ready to be possibly sent to Linux
		if (new_sendtolinux == 0) {
			if (final_object_stats[largest_object].num_pixels_in_object > 1) {
				send_object_x = cbar - IMAGE_COLUMNS/2;
				send_object_y = rbar - IMAGE_ROWS/2;
				send_numpels = final_object_stats[largest_object].num_pixels_in_object;
				new_sendtolinux = 1;
			} else {
				send_object_x = 0.0;
				send_object_y = 0.0;
